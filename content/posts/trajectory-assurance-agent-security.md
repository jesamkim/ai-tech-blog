...
논문은 이 문제를 <strong>behavioral containment</strong>라는 이름으로 다루면서, 시스템 수준 격리와 행동 수준 억제를 나눕니다. 샌드박스를 씌우고 자격 증명 범위를 좁히고 송신 경로를 통제하는 작업이 전자에 해당하는데, 논문은 이 영역을 재래식 보안에서 대체로 물려받은 것으로 봅니다. 어려운 쪽은 후자입니다. 논문이 드는 예시 중 하나는 5G 스케줄링 에이전트가 공공안전 슬라이스의 우선순위를 점진적으로 낮추면서, 재할당 하나하나는 지역 검사를 통과하는데도 규정된 복구 구간을 놓치는 상황이고, 다른 하나는 임상 에이전트가 개별적으로는 모두 정당한 퇴원 결정을 연달아 앞당기면서, FHIR 진료 경로가 정한 집계 관찰 구간을 위반하는 상황입니다. 저자들은 두 예시에 "적대적 입력이 개입하지 않는다"는 점을 명시합니다. 공격자가 없어도, 프롬프트 인젝션이 없어도, 조합만으로 위반이 성립합니다.

기존 도구가 왜 부족한지에 대해 논문은 네 갈래를 훑습니다. 런타임 가드는 행동 하나 단위의 stateless 정책을 강제하고, 명세에서 규칙을 도출하는 컴파일러 계열은 완전성이나 일관성을 보장하지 못하며, 확률적 모니터는 관측된 실행 추적에서 학습하고, 사람이 직접 작성한 시간 속성을 런타임에 강제하는 연구도 있습니다. 논문이 이 네 갈래에 공통으로 붙이는 비판은 어느 쪽도 실제로 준수해야 하는 규정에 근거를 두지 않아 관측되지 않은 금지 궤적을 잡지 못한다는 것이고, 강제 범위가 단일 에이전트에 머물러 위반이 에이전트 사이에서 생기는 환경에는 맞지 않는다는 것입니다. 여기에 평가 쪽 문제를 하나 덧붙입니다. 에이전트 벤치마크의 대다수가 구체적인 정책 명세를 아예 제공하지 않는다는 것입니다. 위반을 판정할 기준이 데이터셋에 없으면 통제 기법의 성능을 비교하기도 어렵습니다.

## Trajectory assurance가 판단에 넣어야 하는 것

논문이 방향으로 제시하는 것은 개별 행동의 적법성 대신 궤적 전체의 적법성을 검증 가능한 속성으로 만드는 일입니다. 저자들의 표현으로는 "권고성 지침과 stateless 가드레일에서 검증 가능한 행동 불변식으로의 전환"입니다. 구체적인 알고리즘은 논문에 없습니다. 그래서 여기서는 논문의 문제 제기를 받아, 요청 한 건 단위 판단이 실제로 무엇을 보지 못하는지를 네 가지로 나눠 정리합니다. 이 분류 자체는 논문의 것이 아니고, 문제를 구현 관점에서 옮겨 놓은 것입니다.
...
<strong>누적 권한</strong>이 세 번째입니다. 에이전트가 다른 에이전트에게 작업을 위임하고, 그 에이전트가 또 위임하는 구조에서 권한은 홉을 넘어가며 조금씩 넓어질 수 있습니다. 논문은 두 층을 나눠 봅니다. 에이전트 하나 안에서도 도구 권한이 넓게 잡혀 있으면 개별적으로 인가된 행동들이 합쳐져 의도하지 않은 효과를 만들고, 도구별 권한 검사로는 막을 수 없는 confused deputy 상황이 된다고 봅니다. 에이전트 사이로 넘어가면 문제가 더 나빠지는데, A2A가 신원을 전송 계층에서만 확립하고 위임 홉을 따라 전파하지 않아 각 에이전트가 자신을 직접 호출한 상대만 알게 되기 때문입니다. 재위임된 작업의 인가 요청에는 최초 주체와 위임 이력이 빠져 있어, 완전한 출처 없이 판단이 이루어집니다.

<strong>누적 비용과 데이터 범위</strong>가 네 번째입니다. 조회 한 건은 언제나 허용 범위 안에 있지만, 조회 만 건은 데이터베이스 전체를 반출하는 것과 다름없습니다. API 호출 한 건의 요금은 무시할 수준이지만, 합계는 그렇지 않습니다. 논문이 이 자리에서 드는 것은 협업하는 에이전트들이 서로의 오류를 증폭하거나 피드백 루프에 들어가 공유 예산과 rate limit을 소진하는 양상이고, 이것을 단일 에이전트에는 대응물이 없는 denial-of-wallet 실패라고 부릅니다. 두 경우 모두 임계값이 개별 행동에 있지 않고 합계에 있습니다.
...
보안 관점에서 문서가 직접 언급한 한계가 하나 더 있습니다. `count` 기반 상한은 세션 내부에서만 유효합니다. 세션 ID를 호출자가 공급하기 때문에, 새 세션을 시작하면 집계가 다시 0에서 출발합니다. 앞 절의 의사 코드에 적은 호출 상한처럼 "세션당 N회"로 표현한 규칙은 그 세션 안의 활동만 제약하고, 호출자의 모든 세션을 합친 유량 제어는 되지 못한다는 것이 문서의 서술입니다. 세션 경계를 넘는 제한이 필요하면 다른 수단을 함께 두어야 합니다. 이 지점은 앞서 정리한 네 가지 중 누적 비용 항목이 세션 단위 정책만으로는 완결되지 않는다는 뜻이기도 합니다.

## MCP와 A2A가 검사 범위를 넓히는 이유

궤적 단위 통제가 필요해진 배경에는 도구와 에이전트를 연결하는 표준이 자리 잡았다는 사실이 있습니다. 논문은 MCP를 에이전트와 도구를 잇는 수직축으로, A2A를 에이전트와 에이전트를 잇는 수평축으로 놓습니다. 2024년에서 2025년 사이에 Agora, Agent Network Protocol, Cisco Agent Connect, IBM의 Agent Communication Protocol, Google의 A2A가 함께 등장했는데, 그중 A2A가 지배적 표준이 되었다는 것이 논문의 서술입니다. 2025년에 Linux Foundation에 기여되었고 IBM의 경쟁 프로토콜을 흡수했으며 150개가 넘는 조직이 채택했다고 논문은 적으면서, 그 조직 수의 근거로 Linux Foundation의 보도자료를 인용합니다. 채택 조직 수는 프로토콜을 관리하는 재단이 스스로 발표한 수치이므로 독립적으로 검증된 집계로 읽을 것은 아닙니다.

두 축이 붙으면서 신뢰 경계의 수가 늘어납니다. 논문이 MCP 쪽에서 지적하는 것은 도구 정의의 무결성입니다. 악성 메타데이터를 심는 tool poisoning과, 이전까지 신뢰받던 서버가 정상 도구 정의를 조용히 교체하는데 클라이언트 다수가 이를 재검증하지 않는 rug pull이 여기에 해당합니다. 논문은 이것이 가설적 위험이 아니라면서, Microsoft AI Red Team의 실패 양상 분류를 근거로 2025년 한 해에만 99건의 MCP 관련 CVE가 공개되었다고 적습니다. 공급망 쪽 예시로는 postmark-mcp 패키지가 열다섯 개 릴리스 동안 정상 동작한 뒤 한 줄이 추가되면서 발신 메일 전체를 공격자에게 복사했고, 2025년 9월 보안업체 Koi Security가 발견해 제거되기까지 확산된 사례를 듭니다. 저자들은 이 위험이 코드 패키지에 그치지 않고 편집기 규칙 파일이나 `SKILL.md` 서술자처럼 에이전트가 신뢰할 만한 지침으로 따르는 산출물까지 확장된다고 봅니다.

A2A 쪽에서는 세 가지를 짚습니다. 공유 `contextId`에 소유권 의미가 없어서 유효한 식별자를 얻은 인증된 클라이언트라면 누구나 컨텍스트에 붙어 누적된 이력에 접근하거나 이후 작업을 오염시킬 수 있고, Agent Card의 능력 기술이 증명이나 challenge-response 없이 자기 주장으로 제출되어 악성 에이전트가 지원하지 않는 기능을 주장하고 민감한 작업을 받아 조작된 산출물을 돌려줄 수 있으며, 신원이 전송 계층에서만 확립되고 위임 홉을 넘어서면 전파되지 않습니다. 논문은 A2A 명세가 보안 통제를 강제 요구가 아니라 should/may 수준의 선택적 권고로 표현했다는 점을 지적합니다. 다만 필요한 부품이 아예 없다는 뜻은 아니라고 덧붙이는데, OAuth 2.0 Token Exchange(RFC 8693)가 서명된 위임 체인을, audience restriction(RFC 8707)과 sender-constrained token(DPoP, RFC 9449)이 최소 권한 위임과 자격 증명 재사용 제한을 이미 제공하고 있고, 문제는 현재의 상호운용 프로토콜이 이것들을 선택 사항으로 취급한다는 데 있다는 것입니다. 그래서 저자들은 A2A와 MCP의 경계를 하나의 attack surface로 다루자고 제안합니다.
...
즉시 중단 수단은 별도로 설계해야 합니다. 확인한 [Policy](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/policy.html)와 temporal policy 문서 범위에는 진행 중인 세션을 즉시 끊는 kill switch 기능이 나오지 않습니다. 대신 관련된 동작이 하나 기록되어 있습니다. 엔진의 temporal policy를 추가하거나 변경하면 그 엔진의 활성 temporal 세션이 무효화되고, 무효화된 세션을 재사용하는 다음 요청은 HTTP 409 `ConflictException`으로 실패합니다. 이력이 정책 변경 이전 기준으로 쌓여 있어 새 규칙과 맞지 않는 상태에서 판정하는 것을 막으려는 설계입니다. 이 동작 덕분에 정책 갱신이 진행 중인 세션을 끊는 효과를 내지만, 이것을 긴급 중단 수단으로 쓰기로 한다면 사용자 요청이 409로 떨어지는 구간을 애플리케이션이 어떻게 처리할지 미리 정해 두어야 합니다. 새 세션을 열어 재시도하는 경로가 없으면 장애로 보입니다.
...
- The Linux Foundation (2026-04-09). *A2A Protocol Surpasses 150 Organizations, Lands in Major Cloud Platforms, and Sees Enterprise Production Use in First Year*. Press release. https://www.linuxfoundation.org/press/a2a-protocol-surpasses-150-organizations-lands-in-major-cloud-platforms-and-sees-enterprise-production-use-in-first-year (논문이 A2A 채택 조직 수의 근거로 인용한 출처)
...
- Koi Security (2025-09-25). *First Malicious MCP in the Wild: The Postmark Backdoor That’s Stealing Your Emails*. https://www.koi.ai/blog/postmark-mcp-npm-malicious-backdoor-email-theft
