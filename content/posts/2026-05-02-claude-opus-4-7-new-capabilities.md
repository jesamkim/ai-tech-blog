...
Opus 4.7의 새 capabilities 중 가장 실용적인 건 [effort 파라미터](https://platform.claude.com/docs/en/build-with-claude/effort)입니다. 공식 문서는 effort를 "요청 하나에 모델이 얼마나 많은 계산을 쓸지 제어하는 버튼"으로 정의합니다. 값은 `none`, `low`, `medium`, `high`, `xhigh` 다섯입니다. 4.7에서 새로 생긴 건 `xhigh`.
...
6. <strong>기본적으로 tool call을 덜 발생시킴</strong>. effort를 올리면 tool call 빈도가 다시 올라갑니다. Claude Code나 Cursor 같은 agent 하니스에서 서브 에이전트 호출이 확 줄었다면 이건 하니스 버그가 아니라 모델 동작 변경입니다. 필요하다면 effort를 `xhigh`로 올리는 것이 답입니다.
