...
실제 활용 사례를 보면:
- <strong>Spotify</strong>: 고객 지원 티켓 중 단순 반복 문의(계정 복구, 플레이리스트 오류)를 에이전트가 처리합니다. 상담원은 복잡한 케이스에만 집중하게 됐습니다.
- <strong>Novo Nordisk</strong>: 수천 개의 임상 연구 문서를 검색하고 요약하는 데 에이전트를 씁니다. 연구원이 필요한 정보를 찾는 시간이 줄었습니다.
- <strong>금융 서비스 회사들</strong>: 계약서 초안 작성, 컴플라이언스 문서 검토, 고객 이메일 응답 생성 등에 사용합니다.
...
Claude Cowork를 비판하려는 게 아닙니다. 기술 방향은 맞습니다. 다만 기업 환경에는 넘어야 할 장벽들이 있습니다.
...
EC2로 띄워도 되지만, Lightsail이 더 간단합니다. 고정 IP, SSH 접속, 방화벽 설정이 콘솔 몇 번 클릭으로 끝납니다. 프로덕션 규모가 커지면 그때 ECS나 EKS로 마이그레이션해도 됩니다.
...
S3 버킷에서 문서 읽기, RDS 데이터베이스 쿼리, 사내 REST API 호출 같은 걸 MCP 서버로 만들어서 에이전트에 연결하면 됩니다. OpenClaw 공식 문서에 MCP 서버 작성 가이드가 있습니다.

동작 방식은 간단합니다. 에이전트가 Bedrock(Claude)에 질문을 보냅니다. Claude가 "S3에서 계약서를 읽어와야겠네"라고 판단하면 MCP 호출을 요청합니다. 에이전트는 MCP 서버를 통해 S3 API를 호출하고 결과를 Claude에 다시 보냅니다. 모든 통신이 VPC 안에서만 일어납니다.
