...
description: AWS Solutions Architect 4년 차, 연대 AI 대학원 2학년. 새 학기 분산학습 수업을 들으며 다시 기본으로 돌아갑니다. 딥러닝이 정확히 무엇을 하는 건지, 왜 작동하는지 차근차근 정리했습니다.
...
```mermaid
graph RL
    E[Loss 계산] --> D[Layer 3 Gradient]
    D --> C[Layer 2 Gradient]
    C --> B[Layer 1 Gradient]
    B --> A[각 가중치를 얼마나 수정해야 하는지 파악]
```
...
graph TB
    A[전체 데이터 5만 장] --> B[섞기]
    B --> C[Mini-batch 1: 100장]
    B --> D[Mini-batch 2: 100장]
    B --> E[Mini-batch 3: 100장]
    B --> F[...]
    C --> G[Gradient 계산 + 업데이트]
    G --> D
    D --> H[Gradient 계산 + 업데이트]
    H --> E
    E --> I[Gradient 계산 + 업데이트]
    I --> F
    F --> J[1 Epoch 완료]
    J --> B
```
...
- 한동준 교수, "AI 분산 학습 및 추론" 강의 자료, 연세대학교, 2026
