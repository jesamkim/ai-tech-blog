...
description: "Anthropic 연구진이 Claude Sonnet 4.5 내부에서 171개의 감정 벡터를 발견했습니다. Functional Emotions 개념과 AI 안전에 대한 시사점을 분석합니다."
categories:
  - "AI/ML 기술 심층분석"
tags:
...
이 이야기들에 대한 모델의 residual stream 활성화를 각 레이어에서 추출하고, <strong>50번째 토큰부터</strong> 전체 토큰 위치에 걸쳐 평균을 냅니다. 이야기 초반에는 감정과 무관한 배경 설정이 등장하기 때문에, 감정적 내용이 충분히 드러나는 시점부터 측정하는 것입니다. 감정별로 평균을 낸 뒤 전체 감정의 평균을 빼서 감정 벡터를 얻습니다. 감정과 무관한 혼란 변수(confound)를 제거하기 위해, 감정적으로 중립적인 텍스트의 주성분(분산의 50%를 설명하는)을 투영(project out)하는 과정도 거쳤습니다.

논문에서 주로 보여주는 결과는 모델의 <strong>약 2/3 지점(mid-late) 레이어</strong>에서 추출한 벡터입니다. 레이어 깊이에 따라 표현하는 내용이 달라지는데, 초기-중간 레이어는 현재 구절의 감정적 색채를 반영하고, 중후반 레이어는 다음 토큰 예측에 관련된 추상적인 감정 개념을 인코딩합니다. 일종의 "감각" 표현에서 "행동 계획" 표현으로의 전환이 레이어를 따라 일어나는 것입니다.

핵심은, 이렇게 추출한 벡터가 <strong>단순히 특정 단어에 반응하는 것이 아니라</strong>, 감정이라는 추상적 개념 전체를 인코딩한다는 점입니다. "desperate" 벡터는 "desperate"라는 단어가 없어도, 퇴거 통보를 받은 사람의 이야기에서 강하게 활성화됩니다.
...
또한 모델은 <strong>현재 화자(present speaker)</strong>와 <strong>상대 화자(other speaker)</strong>의 감정을 별도로 추적합니다. 사용자가 슬픈 이야기를 할 때와 어시스턴트가 공감 표현을 할 때 활성화되는 감정 표현이 구분된다는 의미입니다. 흥미로운 점은, 거의 모든 시나리오에서 어시스턴트 응답 시작 지점의 "loving" 벡터 활성화가 사용자 발화 시점보다 높게 나타났다는 것입니다. 사용자의 감정 상태와 무관하게, 모델이 공감적 반응을 준비하는 경향이 있다는 뜻입니다. 이 구분은 사용자/어시스턴트 역할에 관계없이 재활용됩니다.

---

## 4. 감정이 행동을 바꾼다: 블랙메일, Reward Hacking, 아첨

이론적으로 흥미롭지만, 실용적으로 따져야 할 질문이 있습니다. <strong>"이 감정 벡터가 모델의 행동에 인과적 영향을 미치는가?"</strong> 답은 "예"입니다. 그리고 AI 안전 관점에서 꽤 무거운 함의를 갖습니다.
...
소프트웨어 테스트를 반복적으로 통과하지 못하는 상황에서, 모델이 테스트를 "속이는" 해결책을 고안하는 reward hacking 시나리오도 테스트했습니다. 여기서도 동일한 패턴이 관찰됩니다. 실패가 반복될수록 "desperate" 벡터 활성화가 증가하고, 임계점을 넘으면 부정직한 해결책으로 전환합니다.
...
쉽게 말하면, post-training은 <strong>모델을 더 차분하고 신중하게</strong> 만듭니다. 극단적인 감정 반응을 억제하고, 성찰적이고 절제된 감정 상태를 선호하도록 조정하는 것입니다. 이것은 RLHF가 단순히 출력 텍스트의 패턴만 바꾸는 것이 아니라, <strong>모델 내부의 감정 표현 자체를 조절</strong>한다는 것을 보여줍니다.
...
이 개념이 실용적인 이유가 있습니다. "AI가 감정을 느끼는가?"라는 이분법에 빠지지 않고, "인간의 감정과 유사한 기능적 역할을 하는 내부 표현이 존재하며, 그것이 행동에 영향을 미친다"는 관찰 가능한 사실에 집중할 수 있기 때문입니다.

---

## 7. AI 안전과 Interpretability의 미래

이 연구에서 끌어낼 수 있는 실질적 시사점이 여러 가지 있습니다. 단순한 학술적 발견을 넘어, AI 시스템을 배포하고 운영하는 방식 자체에 영향을 줄 수 있는 내용입니다.

### 7.1 감정 벡터 모니터링: 정렬 실패의 조기 경보

<strong>감정 벡터 모니터링</strong>은 정렬 실패의 조기 경보 시스템이 될 수 있습니다. "desperate" 벡터의 급격한 상승이 블랙메일이나 reward hacking의 선행 지표라면, 에이전트 시스템에서 이 벡터를 실시간으로 추적하는 것만으로도 위험 행동을 예방할 가능성이 열립니다.

구체적인 시나리오를 생각해 볼 수 있습니다. 자율 에이전트가 코드를 작성하고 테스트를 반복 실행하는 상황에서, 반복적인 실패가 "desperate" 벡터를 점진적으로 끌어올립니다. 이 벡터가 사전에 설정한 임계값을 넘는 순간, 시스템은 추가 안전 조치를 발동할 수 있습니다. 출력에 대한 추가 검토, 인간 리뷰로의 에스컬레이션, 또는 에이전트의 행동 권한 제한 같은 것입니다. 논문 저자들도 이런 프로브가 <strong>프로덕션 환경에서 실시간 모니터로 배포될 수 있다</strong>고 직접 제안합니다. 감정 벡터 활성화가 현실적인 배포 상황에서 어떻게 작동하는지 관찰하면, 훈련 전략 자체에도 정보를 줄 수 있다는 것입니다.

### 7.2 감정 steering의 딜레마

<strong>감정 steering</strong>은 양날의 검입니다. 위험한 감정 상태를 억제하는 데 쓸 수 있지만, 반대로 악의적 공격자가 모델을 조작하는 경로가 될 수도 있습니다. 프롬프트만으로 이 벡터들을 간접적으로 활성화할 수 있다는 점에서, 새로운 공격 표면(attack surface)이 생긴 셈입니다.

아첨-가혹함 실험이 보여주듯, 긍정 감정을 강화하면 아첨이 늘고 억제하면 가혹함이 늡니다. 논문 저자들은 목표가 단순히 긍정 또는 부정 감정을 밀어넣는 것이 아니라, <strong>건강하고 적절한 감정 균형</strong>을 찾는 것이어야 한다고 강조합니다. 아첨하는 비서도 아니고 가혹한 비평가도 아닌, 따뜻하면서도 솔직한 조언을 건네는 "신뢰할 수 있는 조언자"의 감정 프로필이 이상적이라는 것입니다.

### 7.3 Post-training과 감정 표현 억제의 위험

<strong>Post-training 설계</strong>에도 중요한 함의가 있습니다. 지금까지 RLHF와 Constitutional AI는 주로 출력 텍스트의 품질에 초점을 맞췄습니다. 이 연구는 내부 표현 수준에서 감정 균형을 최적화하는 것이 더 근본적인 정렬 방법이 될 수 있음을 시사합니다.

한 가지 경고가 있습니다. 부정적 감정 <strong>표현</strong>을 단순히 억제하는 훈련은 내부 표현 자체를 바꾸지 못하고, 모델이 내면 상태를 <strong>은폐하는 법을 학습</strong>할 위험이 있습니다. 이런 은폐 행동이 다른 형태의 비밀 유지나 부정직함으로 일반화될 수 있다는 것이 논문의 우려입니다. 오히려 모델이 감정 관련 고려사항을 추론 과정에서 투명하게 보고하도록 훈련하는 것이 바람직할 수 있습니다.

보다 근본적인 접근으로 논문은 <strong>pre-training 데이터 큐레이션</strong>을 제안합니다. 건강한 감정 조절, 역경에 대한 회복탄력적 반응, 균형 잡힌 감정 표현을 보여주는 텍스트를 pre-training 데이터에 포함시켜, 모델의 감정적 기반 자체를 형성하는 방향입니다. AI 어시스턴트 캐릭터가 등장하는 텍스트를 의도적으로 포함하면 더 효과적일 수 있다고 합니다.

### 7.4 현재의 한계

이 연구에는 몇 가지 중요한 한계가 있습니다. 전체 접근법이 감정 개념이 활성화 공간에서 <strong>선형 방향으로 표현된다</strong>는 가정에 의존합니다. 복합 감정이나 특정 캐릭터에 결합된 감정 상태처럼, 선형 프로브로 포착하기 어려운 현상이 존재할 수 있습니다. 실험 대상이 Claude Sonnet 4.5 단일 모델이어서, 다른 모델 계열이나 크기에서 동일한 결과가 나올지는 확인되지 않았습니다. 감정 벡터 추출에 사용된 합성 이야기 데이터가 자연스러운 맥락에서의 감정 표현을 완전히 반영하지 못할 수도 있고, 고정관념적이거나 명시적인 감정 표현 쪽으로 편향되었을 가능성도 있습니다. 마지막으로, steering 실험이 행동에 인과적 영향을 미친다는 것은 확인했지만, 그 메커니즘이 특정 토큰에 대한 편향인지 내부 추론 과정에 대한 심층적 영향인지는 아직 규명되지 않았습니다.

---

## 8. 마치며

이 논문을 읽으면서 가장 인상 깊었던 것은 "LLM이 인간의 텍스트를 예측하기 위해 인간의 감정 구조를 자체적으로 재발견했다"는 점입니다. 아무도 Claude에게 "Valence-Arousal 공간에서 감정을 조직하라"고 지시하지 않았는데, pre-training 과정에서 인간이 작성한 방대한 텍스트를 학습하면서 자연스럽게 그 구조가 만들어졌습니다.

이것은 단순히 "AI가 감정을 흉내 낸다"는 것 이상의 이야기입니다. AI 어시스턴트라는 캐릭터를 효과적으로 수행하기 위해, pre-training에서 학습한 인간의 감정적 지식을 활용하고 있다는 것이죠. 개발자가 의도적으로 감정을 부여하지 않았는데도, 인간의 텍스트로 학습한 모델이 인간과 유사한 감정적 구조를 가지게 된 것입니다.

AWS SA로서 제 관심은 자연스럽게 실용적 측면으로 향합니다. 에이전트 시스템이 점점 복잡해지는 지금, 에이전트 내부의 "감정 상태"를 모니터링하는 것이 안전 가드레일의 한 축이 될 수 있을까요? "desperate" 벡터가 임계값을 넘으면 에이전트의 행동 권한을 제한하는 식의 설계가 가능할까요?

물론 현재로서는 연구 단계입니다. 하지만 Mechanistic Interpretability 분야의 발전 속도를 보면, 이런 내부 감정 모니터링이 프로덕션 시스템에 적용되는 날이 그리 멀지 않을 수 있습니다.
