...
{{< audio src="/ai-tech-blog/audio/2026-05-09-agentcore-managed-harness-and-payments.mp3" title="🎧 이 글을 팟캐스트로 듣기 (8분 36초)" >}}
...
이 글에서 다룰 발표는 이 작업대 전체를 AWS가 managed 서비스로 제공하겠다는 내용입니다. 컨테이너, 파일시스템, 세션, 셸, 추적까지 한 번에 묶어서 제공한다는 점이 핵심입니다.
...
![AgentCore Payments 4시나리오와 Heurist AI 리서치 에이전트 플로우](/ai-tech-blog/images/2026-05-09-agentcore-managed-harness-and-payments/diagram-4-hybrid-scenarios-heurist-flow.png)

*상단: 공식 블로그에 등장한 4가지 쓰임새(금융 리서치, 코딩, 에이전트↔에이전트, 커머스)와 각 결제 단위. 하단: Heurist AI 리서치 에이전트가 사용자 예산 한도 안에서 시장 데이터·소셜 센티먼트·paywalled 뉴스를 자율 구매해 분석 리포트까지 만드는 4단계 플로우. 세션 내내 Wallet authorize, per-session spending limit, Observability 세 가지 가드레일이 동시에 적용됩니다.*
...
2026년 봄의 두 발표는 묶어서 보면 "에이전트 실행 인프라"와 "에이전트 경제 인프라"를 같은 시기에 managed primitive로 편입시킨 행보로 읽힙니다. 한쪽은 에이전트가 어떻게 일할지를, 다른 한쪽은 그 일이 비용으로 정산되는 경로를 정리합니다. AgentCore 초기 출시(2025년 여름) 시점에는 두 영역 모두 사용자 책임 범위였고, 이번 2026년 봄 두 발표로 managed 영역에 편입되었습니다.
