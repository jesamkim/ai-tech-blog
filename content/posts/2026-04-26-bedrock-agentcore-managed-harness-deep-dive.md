...
모델 provider는 <strong>Amazon Bedrock, OpenAI, Anthropic(직접 API), Google Gemini</strong> 네 곳입니다. Bedrock에서는 Claude Sonnet 4.6이 기본값이고 Opus 4.6도 선택할 수 있습니다. OpenAI는 GPT-5.4, Gemini는 Gemini 2.5 Pro 계열을 바라봅니다. CLI에서는 `agentcore create --model-provider [Bedrock|OpenAI|Anthropic|Gemini]` 플래그로 provider를 선택합니다.
...
- HelloAgent 기본 템플릿의 모델은 `global.anthropic.claude-sonnet-4-5-20250929-v1:0`입니다. `harness.json` 기준 기본값(Sonnet 4.6)과는 다를 수 있으므로 CLI가 생성한 `src/model/load.py` 값을 기준으로 확인하세요.
