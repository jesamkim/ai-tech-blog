...
2024년 이후 에이전트 프레임워크는 쏟아졌습니다. LangChain, LangGraph, CrewAI, AutoGen, Strands Agents, Google ADK, OpenAI Agents SDK. 각자 다른 추상화를 제공하지만, 실제로 프로덕션에 올려본 팀들은 비슷한 문제를 공유합니다.
...
AgentCore는 2025년 7월 AWS Summit New York에서 <strong>Runtime, Memory, Gateway, Browser, Code Interpreter, Identity, Observability</strong> 7개 서비스로 프리뷰가 공개되었고, 2025년 10월 13일 GA되었습니다. 각 서비스는 각자 강력했지만, 사용하려면 Python SDK로 AgentCore Runtime을 감싸고 Gateway를 IAM으로 연결하고 Memory를 RUNTIME 전략으로 설정하는 식의 조립이 필요했습니다. 조립의 난이도는 여전히 팀 몫이었습니다.
...
Managed Harness의 <strong>기본 엔진</strong>은 AWS 오픈소스 프레임워크인 <strong>Strands Agents</strong>입니다. [SiliconANGLE 보도](https://siliconangle.com/2026/04/22/aws-accelerates-ai-agent-development-amazon-bedrock-agentcore/)가 이 점을 분명히 합니다. 단, CLI의 `agentcore create --agent-framework` 옵션은 Strands 외에도 <strong>LangChain/LangGraph, Google ADK, OpenAI Agents, AutoGen, CrewAI</strong> 등 총 6개 프레임워크를 지원합니다(`@aws/agentcore` v0.11.0 기준). 기본값을 그대로 쓰면 루프 제어, 도구 바인딩, 에러 복구, 스트리밍 출력이 Strands의 ReAct 스타일 루프 위에서 실행됩니다.
...
모델 provider는 <strong>Amazon Bedrock, OpenAI, Anthropic(직접 API), Google Gemini</strong> 네 곳입니다. CLI에서는 `agentcore create --model-provider [Bedrock|OpenAI|Anthropic|Gemini]` 플래그로 provider를 선택합니다. 각 provider의 구체적인 모델 ID와 기본값은 CLI 버전과 템플릿에 따라 달라지므로 `agentcore create` 결과물의 `src/model/load.py`(또는 동등 위치)에서 확인하는 편이 안전합니다. 참고로 2026-04-26 기준 `@aws/agentcore` v0.11.0 HelloAgent 기본 템플릿은 `global.anthropic.claude-sonnet-4-5-20250929-v1:0`을 사용합니다.
...
CLI는 `npm install -g @aws/agentcore`로 설치합니다. 2026-04-26 기준 npm에 올라온 버전은 `0.11.0`(프리뷰)이고, 소스는 [aws/agentcore-cli GitHub](https://github.com/aws/agentcore-cli)에 공개되어 있습니다.
...
`agentcore.json`은 프로젝트 메타데이터와 하니스 목록을 담습니다. `aws-targets.json`은 어느 계정의 어느 리전에 배포할지를 지정합니다. `cdk/`는 AWS가 생성한 CDK 스택으로, 실제 리소스 프로비저닝을 여기서 수행합니다. IaC는 `agentcore create --iac [CDK|Terraform]` 옵션으로 <strong>CDK와 Terraform 중 선택 가능</strong>합니다(`@aws/agentcore` v0.11.0 기준 양쪽 모두 지원).

하니스 정의는 `app/<하니스명>/harness.json`에 놓습니다. 여러 하니스를 하나의 프로젝트에 두고 독립적으로 배포할 수 있습니다.

참고로 `@aws/agentcore` v0.11.0 공개 CLI가 생성하는 실제 구조는 위 블로그 예시의 `harness.json` 형태가 아니라 Strands 기반 Python 코드 템플릿에 더 가깝습니다. `HelloAgent/.bedrock_agentcore.yaml`(프로젝트 설정) + `HelloAgent/src/main.py`(Strands Agent 구성) + `pyproject.toml` 조합으로, 모델·시스템 프롬프트·도구 선언이 `main.py` 내 `Agent(model=..., system_prompt=..., tools=[...])` 호출로 표현됩니다. ClassMethod 기사의 `harness.json` 경로는 별도 내부 분기 또는 프리뷰 버전일 가능성이 있으므로 실 사용 시점의 CLI 출력 기준으로 구조를 확인하는 편이 안전합니다.
...
AgentCore는 <strong>microVM</strong> 위에서 세션을 돌립니다. AWS 공식 devguide는 "Each user session in AgentCore Runtime receives its own dedicated microVM with isolated compute, memory, and filesystem resources"로 microVM 사용을 명시합니다. 다만 hypervisor 종류(Firecracker인지 Nitro 계열인지)는 현재 공식 문서에 기재되어 있지 않고, 필자가 직접 돌려본 이번 실험에서도 응답 헤더·메트릭·로그 어디에도 hypervisor 힌트는 노출되지 않았습니다. 세션마다 VM 한 대를 잡아 다른 세션과 메모리·파일시스템·프로세스 네임스페이스를 모두 분리한다는 점은 사실이고, 이것이 AgentCore 격리 모델의 핵심입니다.
...
AgentCore Runtime 자체는 2025년 7월 AWS Summit New York에서 공개된 에이전트 호스팅 서비스입니다. Managed Harness는 Runtime을 걷어낸 게 아니라 그 <strong>위</strong>에 얹힌 매니지드 레이어입니다. 배포된 하니스는 내부적으로 Runtime 엔드포인트로 노출되고, 호출 API도 Runtime과 동일한 `InvokeAgentRuntime`을 씁니다.
...
관측성은 [AgentCore Observability](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability.html)가 기본 제공합니다. CloudWatch 로그, OpenTelemetry 트레이스, 세션별 지표가 콘솔에서 조회됩니다. 실측 결과 메트릭 네임스페이스는 <code>AWS/Bedrock-AgentCore</code>(하이픈 포함)이고, 로그 그룹은 <code>/aws/bedrock-agentcore/runtimes/&lt;RUNTIME-ID&gt;-DEFAULT</code> 패턴으로 자동 생성됩니다. 기본 메트릭 8종(`Latency`, `SystemErrors`, `UserErrors`, `Throttles`, `Sessions`, `Invocations`, `Errors`, `Duration`)이 Resource/Operation/Name 차원으로 제공되므로 별도 에이전트 설치 없이 바로 알람을 걸 수 있습니다. 장시간 세션이 많아지면 Observability 대시보드가 디버깅의 1차 창구가 됩니다.
...
SiliconANGLE 기사는 VTEX 등 얼리 어답터 고객 피드백을 정리하면서 AgentCore Managed Harness와 CLI가 에이전트 기능의 출시 시간을 단축했다고 전합니다. 다만 필자가 확인한 범위에서는 Rodrigo Moreira 등 개별 인용구의 1차 출처를 AWS 공식 자료에서 직접 재확인하지 못했습니다. 프리뷰 단계의 초기 신호로 참고하되, 구체적 문구는 SiliconANGLE 보도를 원출처로 취급하는 편이 안전합니다.
...
프리뷰에서 GA로 넘어가는 구간에 개발자가 준비할 항목도 정리해둘 만합니다. API 시그니처와 `harness.json` 스키마는 프리뷰 기간 중 변경될 수 있습니다. CLI 릴리스 노트와 [AgentCore 개발자 가이드](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agentcore-get-started-cli.html)를 북마크해두면 변경 포인트를 빠르게 추적할 수 있습니다.
...
2026-04-26에 `@aws/agentcore` CLI를 실제 AWS 계정(us-west-2)에 올려 주요 동작을 확인했습니다. 배포–호출–정리까지 전체 사이클을 직접 돌려본 결과 중 블로그 내용과 어긋나는 부분과, 실측으로 확인된 수치를 정리합니다.

<strong>확인된 것</strong>

- 배포 소요: <strong>32초</strong>(HelloAgent 기본 템플릿, `direct_code_deploy` 모드).
- 세션 격리: 서로 다른 `sessionId`로 보낸 호출이 컨텍스트를 공유하지 않음. CloudWatch log stream도 `[runtime-logs-session-<id>]` 패턴으로 세션마다 분리되어 생성됨.
- 관측성: 메트릭 네임스페이스 `AWS/Bedrock-AgentCore`, 로그 그룹 `/aws/bedrock-agentcore/runtimes/<RUNTIME-ID>-DEFAULT` 패턴 실측 일치. `Latency`, `SystemErrors`, `UserErrors`, `Throttles` 등 기본 메트릭이 별도 에이전트 없이 emit됨. 엔트리포인트가 `["opentelemetry-instrument", "main.py"]`로 자동 래핑되어 OTel 트레이스가 기본 활성화됨.
- `agentcore destroy --force`로 Agent Runtime, S3 deployment/source artifacts, 실행 IAM 역할이 일관되게 정리됨. 런타임 CloudWatch 로그 그룹은 수명주기 보존 정책에 따라 잔존.

<strong>블로그 서술과 달랐던 부분</strong>

- `harness.json` 중심의 선언 구조는 공개 CLI(`@aws/agentcore` v0.11.0)에서는 생성되지 않습니다. 실제 템플릿은 `.bedrock_agentcore.yaml` + Strands 기반 `src/main.py` 구조입니다. 개념적으로는 여전히 `model + systemPrompt + tools` 세 축이 에이전트를 정의하지만, 표현 매체가 JSON이 아니라 Python 코드입니다.
...
- HelloAgent 기본 템플릿의 모델은 `global.anthropic.claude-sonnet-4-5-20250929-v1:0`입니다. 기본값은 CLI 버전에 따라 달라질 수 있으므로, 이식성이 중요한 경우 `src/model/load.py`(또는 동등 위치)에서 명시적으로 확인하세요.
