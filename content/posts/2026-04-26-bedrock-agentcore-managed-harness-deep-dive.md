...
### 기본 템플릿은 Strands, 다만 프레임워크 중립이다

Managed Harness의 기본 템플릿은 AWS 오픈소스 프레임워크인 <strong>Strands Agents</strong>입니다. [SiliconANGLE 보도](https://siliconangle.com/2026/04/22/aws-accelerates-ai-agent-development-amazon-bedrock-agentcore/)가 이 조합을 가장 먼저 소개한 것도 그 때문입니다. 다만 AgentCore를 "Strands 전용 런타임"으로 오해하면 안 됩니다. 실측 결과 AgentCore starter-toolkit은 `strands/`, `langchain_langgraph/`, `crewai/`, `openaiagents/`, `googleadk/`, `autogen/` 템플릿을 1급 지원합니다. `BedrockAgentCoreApp` 자체는 `/invocations` POST 핸들러를 감싸는 Starlette 래퍼일 뿐이고, 내부에서 어떤 프레임워크를 쓰든 서명만 맞으면 됩니다. 즉 Strands는 Managed Harness의 기본값일 뿐, 강제 엔진은 아닙니다.
...
Strands 자체가 궁금하다면 [AWS Developer Blog](https://aws.amazon.com/blogs/opensource/introducing-strands-agents-an-open-source-ai-agents-sdk/)와 [GitHub 저장소](https://github.com/strands-agents/sdk-python)를 참고하면 됩니다. Managed Harness 사용자는 Strands API를 직접 다루지 않지만, 동작 모델을 이해해두면 시스템 프롬프트 설계와 도구 반환값 형태를 잡을 때 도움이 됩니다.
...
## 실전 실험: 직접 돌려본 AgentCore Runtime

이 글의 상당 부분은 Managed Harness(2026-04-22 프리뷰) 자체가 아니라, 그 아래 깔린 <strong>AgentCore Runtime</strong>(2025-10-13 GA)의 동작 원리를 전제로 합니다. 프리뷰인 Managed Harness를 직접 붙잡기 전에, 필자는 Runtime + Strands 조합으로 별도 실험을 한 번 돌렸습니다. 전수 검증은 아니지만 블로그의 주장 몇 가지를 실측으로 확인·보정하는 데 도움이 되었습니다.

실험 구성은 다음과 같습니다.

- 리전: us-west-2
- SDK: `bedrock-agentcore` 1.2.0, `bedrock-agentcore-starter-toolkit` 0.2.6, `strands-agents` 1.29.0
- 모델: `us.anthropic.claude-sonnet-4-5-20250929-v1:0` (모델 ID는 날짜 포함 형식 필수)
- 배포 모드: `direct_code_deploy` (toolkit 0.2.6 기본값)
- 도구: `add_numbers`, `get_current_time` (자작 Strands @tool)

실측 결과(일부):

| 항목 | 값 |
|---|---|
| 배포 시간 | <strong>27.8초</strong> (configure 0.2s + launch 27.5s + poll 0.1s) |
| Cold invoke latency | 6.57초 (서버 핸들러 3.55초) |
| Warm invoke latency | 3.27초 (서버 핸들러 3.17초) |
| 패키지 크기 | 59.26 MB |
| `idleRuntimeSessionTimeout` | 900초 (15분) |
| `maxLifetime` | 28,800초 (8시간) |
| `requireMMDSV2` | true (IMDSv2 강제) |

배포 시간이 기대치("수 분")보다 훨씬 빠른 이유는 `direct_code_deploy`가 CodeBuild/ECR 경로를 거치지 않기 때문입니다. 클라이언트에서 <code>uv</code>로 <code>aarch64-manylinux2014</code> 타깃 크로스 컴파일을 수행해 ZIP을 S3에 올리는 구조입니다. 즉 로컬에 `uv`가 없으면 <code>RuntimeError: uv is required for direct_code_deploy deployment</code> 에러가 먼저 납니다. 별도로 컨테이너 이미지 배포(`container` 모드)를 선택하면 CodeBuild 경로가 살아나고, 대신 배포 시간은 3~5분대로 늘어납니다. 런타임 자체는 어느 쪽이든 ARM64 Graviton에서 실행됩니다.

세션 격리는 블로그 본문의 주장을 행동 수준에서 재확인했습니다. 두 개의 `runtimeSessionId`로 동시에 "비밀 번호를 기억하라"는 프롬프트를 교차로 넣었을 때 서로의 값이 오염되지 않았습니다. 응답 헤더는 <code>x-amzn-bedrock-agentcore-runtime-session-id</code> 하나만 노출되고, VM ID/host ID 등 내부 식별자는 전혀 보이지 않았습니다.

툴 호출 자체는 정상 동작합니다. CloudWatch 로그에 `Tool #1 get_current_time`, `Tool #2 add_numbers`가 기록되고, 응답 본문에도 계산값(`17+25=42`, `888+1234=2122`)과 UTC 타임스탬프가 담깁니다. 다만 한 가지 주의할 점이 있는데, Strands 1.29 기준 `AgentResult.tool_uses` 필드가 빈 배열로 돌아오는 이슈가 있어 <strong>SDK 응답만 보고 툴 호출 이력을 추적하기는 아직 어렵습니다</strong>. 현재는 CloudWatch 로그로 우회하는 것이 가장 확실합니다. 이 부분은 Strands 쪽 향후 패치로 해결될 가능성이 높습니다.

실험 코드 전체와 아티팩트(JSON 원본, 정리된 결과 리포트, cleanup 스크립트)는 공개 레포에 올려두었습니다. `git clone` 후 `requirements.txt` 설치, AWS 자격 증명 설정만 하면 동일한 플로우를 재현할 수 있습니다.

- 실험 레포: [jesamkim/bedrock-agentcore-harness-experiment](https://github.com/jesamkim/bedrock-agentcore-harness-experiment)

실험 범위는 <strong>AgentCore Runtime + Strands</strong>이고, Managed Harness(4/22 프리뷰)의 `agentcore create/dev/deploy` CLI 플로우나 `harness.json` 스키마는 별도 검증 대상입니다. Managed Harness는 현재 리전이 us-west-2, us-east-1, eu-central-1, ap-southeast-2 4곳으로 제한되고 진입 장벽이 다릅니다. 두 레이어의 차이를 혼동하지 않는 것이 이 글을 읽는 핵심입니다.
