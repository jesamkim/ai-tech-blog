...
Amazon은 2024년 기준 전 세계 풀필먼트 센터에서 75만 대 이상의 로봇을 운용 중이고, 100만 대 돌파를 눈앞에 두고 있습니다. 이 규모의 Fleet Management는 그 자체로 분산 시스템 엔지니어링의 극한입니다. 실제로 써보면 단일 창고 수백 대 수준에서도 스케줄링이 만만치 않은데, 이걸 글로벌 스케일로 돌린다고 생각하면 머리가 아파집니다.
...
가장 흥미로운 연구 방향은 로봇 파운데이션 모델(Robot Foundation Model, RFM)입니다. 언어 모델이 대규모 텍스트로 사전학습 후 다양한 태스크에 적용되듯, 로

## Amazon의 Physical AI 확장 전선 — 물류를 넘어 자율주행과 가정으로

Amazon의 Physical AI 야망은 물류 창고에서 멈추지 않는다. 2020년 약 12억 달러에 인수한 Zoox는 Amazon이 모빌리티 영역에서 꺼내 든 가장 공격적인 카드다.

### Zoox — L5 완전 자율주행이라는 도박

대부분의 자율주행 기업이 L4(특정 구역 내 자율주행)에 집중하는 동안, Zoox는 처음부터 L5(Level 5) 완전 자율주행을 설계 목표로 잡았다. 핸들도 페달도 없는 양방향 대칭 차량(bidirectional vehicle)을 자체 설계했는데, 이 지점에서 Waymo나 Cruise와 접근 방식이 근본적으로 갈린다.

![다이어그램 5](/ai-tech-blog/images/posts/2026-02-17/diagram-5.png)

여기서 눈여겨볼 부분은 AWS 인프라와의 시너지다. Zoox는 실도로 주행 데이터를 AWS 클라우드로 전송하고, 수십억 마일 규모의 시뮬레이션을 AWS GPU 클러스터에서 돌린다. 이 시뮬레이션-투-리얼(Sim-to-Real) 파이프라인은 앞서 살펴본 RoboMaker의 확장판이라 할 수 있다.

2024년 말 기준 Zoox는 라스베이거스, 샌프란시스코, 포스터시티에서 공도 테스트를 진행 중이며, 2025년 상용 로보택시 서비스 런칭을 목표로 잡고 있다.

### 가정으로의 침투 — Astro와 그 너머

물류와 도로 다음 전선은 가정이다. Amazon의 가정용 로봇 Astro는 아직 초기 단계지만, Alexa LLM과 결합하면서 물리 세계를 이해하는 가정용 에이전트 방향으로 진화하고 있다. 실제로 써보면 아직 갈 길이 멀다는 느낌이지만, Ring 보안 카메라 생태계와의 연동 덕분에 가정 내 센서 네트워크는 이미 깔려 있는 셈이다.
...
결국 Amazon의 전략은 명확하다. 창고에서 도로로, 도로에서 가정으로 이어지는 물리 공간의 연속선 위에서 AWS 클라우드를 두뇌로, 각 영역의 로봇을 신체로 삼아 Physical AI 생태계를 수직 통합하겠다는 것이다. 개인적으로 이 규모의 시도를 실행할 수 있는 기업은 현재로서는 Amazon 말고는 떠오르지 않는다.
