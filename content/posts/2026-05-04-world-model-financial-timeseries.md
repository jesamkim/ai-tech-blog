...
코드 작성과 반복 실행은 Claude Code를 러너로 활용했습니다. 다만 모델이 내놓은 수치를 그대로 믿지 않기 위해 <strong>별도 세션의 QA subagent에게 `.npy` 샘플 파일만 넘겨주고 stylized-fact 24개 cell을 bit-exact 수준으로 재계산하게 했습니다.</strong> 메인 파이프라인과 QA가 서로의 결과를 모르는 상태에서 값이 일치해야만 통과시키는 구조였는데, 이걸 설계하고 파라미터를 잡는 과정에서 논문의 수식 하나하나를 다시 따라가 봐야 했습니다.
