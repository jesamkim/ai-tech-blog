...
- 코드 예시 포함 가능 (Python)
- 400~800자 분량
- 다이어그램은 포함하지 마세요 (별도 단계에서 생성됩니다)
- 다이어그램이 필요한 위치에 [DIAGRAM: 설명] 형태의 플레이스홀더만 남겨주세요
- 섹션 제목(## 헤더)부터 시작
...
- 현재 시점은 2026년입니다. "2024년 기준", "2024년 현재" 같은 구식 표현을 쓰지 마세요. 시점이 불확실한 정보는 "발표 당시", "논문 기준" 등으로 시점을 명시하세요"""
- 특정 기업/고객사명(에버랜드, 삼성물산, SSF 등)을 직접 언급하지 마세요. 일반적 표현(대형 테마파크, 건설사, 리테일 플랫폼 등)으로 대체하세요
...
12. 기술 용어의 영어 병기는 유지
13. 문체 통일: 반드시 존댓말(합니다/입니다체)로 통일. "~이다", "~한다", "~없다" 같은 반말 문장이 있으면 존댓말로 변환
14. 이전 섹션과의 연결: 섹션 시작이 갑작스럽지 않도록 자연스러운 전환문 확인
